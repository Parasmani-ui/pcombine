...
const IndustryProductionLine = (props) => {
  ...
  _segments.forEach((segment) => {
    ...
    production[segment.name] = Array(currentQuarter).fill(0);
  });
  ...
  teams.forEach((team) => {
    for (var i = 0; i < currentQuarter; i++) {
      ...
      products.forEach((product) => {
        production[product.target][i] += product.actualProduction;
      });
    }
  });

  const datasets = [];
  for (var prop in production) {
    if (!segments[prop]) {
      delete production[prop];
      continue;
    }

    const _lineData = [];
    for (var i = 0; i < props.currentQuarter; i++) {
      _lineData[i] = production[prop][i];
    }
  ...
  return <Card style={{height: '100%'}}>
    ...
    <IndustryProductionToolbar {...props} />
    <AutoSaveText
        saveKey="text.industry_production_line_heading"
        text={caseStudy.text.industry_production_line_heading || 'Total Industry Production'}
        {...props}
      />
    </Card.Header>
    <Card.Body>
...
const IndustryProductionToolbar = (props) => {
  const caseStudy = props.caseStudy;
...
export default IndustryProductionLine;
